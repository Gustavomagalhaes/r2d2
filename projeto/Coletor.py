...
class Coletor:
    ...
    def classificarTransporte(self,transp):
        for name,p in self.__protocolsTransp.iteritems():
            if isinstance(transp,p):
                print "           Camada de Transporte = "+name
                #self.__arq.write(...)
    ...
    def iniciarColeta(self,arquivo = None,timeMaximo = 60): #Se nao haver nenhum arquivo e nenhum tempo de funcionamento especificado o default é de 60 segundos
        try:
            ...
            for ts, pkt in pcap.pcap(arquivo):
                ...
                print "Pacote %i. Camada de Enlace = Ethernet" %(maxi)
                #self.__arq.write(...)
                red = enl.data
...
coletor = Coletor(expressoesRegulares,"log.txt")
